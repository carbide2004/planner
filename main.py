...
class SummaryWindow(tk.Toplevel):
    ...
    def _build(self):
        ...
        tk.Label(body, text="最近一周完成情况", font=("Microsoft YaHei", 16, "bold"),
                 bg=BG, fg=TEXT).pack(anchor="w", pady=(0, 20))

        c_width = 440
        c_height = 220
        canvas = tk.Canvas(body, width=c_width, height=c_height, bg=SURFACE, bd=0, highlightthickness=0)
        ...
        # 绘制底部轴线
        canvas.create_line(pad_x, c_height - pad_y, c_width - pad_x, c_height - pad_y, fill=BORDER, width=2)
        ...
        for i, (date_str, comp, total) in enumerate(stats):
            ...
            # x轴日期系标签
            canvas.create_text(x, c_height - pad_y + 15, text=date_str, fill=TEXT_DIM, font=("Courier New", 9))
            # 数值标签
            canvas.create_text(x, y - 15, text=str(comp), fill=ACCENT, font=("Courier New", 10, "bold"))
        ...
        for x, y in points:
            canvas.create_oval(x-4, y-4, x+4, y+4, fill=BG, outline=ACCENT, width=2)
            
        tk.Button(body, text="关闭", font=("Microsoft YaHei", 11), bg=SURFACE2, fg=TEXT,
                  activebackground=BORDER, activeforeground=TEXT, bd=0, padx=24, pady=8,
